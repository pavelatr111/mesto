...
const cardList = new Section({
  renderer: (data) => {
    return renderPlaceCard(data);
  }
}, '.elements');

const placePopup = new PopupWithForm(".popup_place", submitCardFormHandler);
...
function submitCardFormHandler(formData) {
  return api
    .renderNewCard(formData.name, formData.link)
    .then((cardData) => cardList.addItem(renderPlaceCard(cardData)))
    .catch((err) => console.log(err));
}
...
const popupWithConfirmation = new PopupWithConfirmation('.popup_delete', handleConfirm, 'Удаление...');
popupWithConfirmation.setEventListeners();

function handleConfirm(card) {
  return api
    .cardDelete(card.getId())
    .then(() => card.remove())
    .catch(err => console.log(err));
}
